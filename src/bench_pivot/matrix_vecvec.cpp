...
template <typename T>
T matrix<T>::get(unsigned int x, unsigned int y) const { return m[x][y]; }

template <typename T>
T &matrix<T>::get(unsigned int x, unsigned int y) { return m[x][y]; }

template <typename T>
void matrix<T>::set(unsigned int x, unsigned int y, T val) { m[x][y] = val; }
