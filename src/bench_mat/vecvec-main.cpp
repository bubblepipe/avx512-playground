...
void bench_mat_vecvec(benchmark::State &state,
        void (*func_ptr)( int, int, std::vector<std::vector<float>> &, std::vector<std::vector<float>> &, std::vector<std::vector<float>> &, std::vector<std::vector<float>> &  )
    ) {
    ...
    for (int i = 0; i < row; i += 1) {
        for (int j = 0; j < col; j += 1) {
            ...
            mat_src3[i][j] = (float(rand())/float((RAND_MAX)));
